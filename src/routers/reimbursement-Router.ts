...
reimbursementRouter.post('',
    ...
    (req, res) => {
        ...
        try {
          ...
          let newType: ReimbursementType;
          let newStatus: ReimbursementStatus;
          for (let i = 0; i < RT.length; i++) {
            if (req.body.type == RT[i].type) {
              newType = new ReimbursementType(RT[i].typeId, RT[i].type);
              console.log(newType);
            }
          }
          for (let i = 0; i < RS.length; i++) {
            if (0 == RS[i].statusId) {
              newStatus = new ReimbursementStatus(RS[i].statusId, RS[i].status);
          ...
          const reburse = new Reimbursement((reinbursements.length + 1), req.session.user.userId, parseFloat(req.body.amount), parseFloat(req.body.dateSubmitted), null, req.body.description, null, newStatus, newType);
          reinbursements.push(reburse);
          UploadNewReimbursement(reburse, res);
          const {reimbursementId, author, dateSubmitted, dateResolved, resolver, status, type} = reburse;
          console.log(reburse);
          const queryString = `reimbursementId=${reimbursementId}&author=${author}&dateSubmitted=${dateSubmitted}&dateResolved=${dateResolved}&resolver=${resolver}&status=${status.status}&type=${type.type}`;
...
authMiddleware(users),
    ...
    async (req, res) => {
        try {
          ...
          const id = req.body.searchReimbursement;
          let locator = 0;
          for (let i = 0; i < reinbursements.length; i++) {
          ...
          const reburse = reinbursements[locator];
          const {reimbursementId, author, dateSubmitted, dateResolved, resolver, status, type} = reburse;
          console.log(reburse);
          const queryString = `reimbursementId=${reimbursementId}&author=${author}&dateSubmitted=${dateSubmitted}&dateResolved=${dateResolved}&resolver=${resolver}&status=${status.status}&type=${type.type}`;
          console.log('Sending User To Specified Reimbursement Page');
...
reimbursementRouter.post('/update',
  ...
  async (req, res) => {
    ...
    try {
      ...
      const queryString = `reimbursementId=${reburse.reimbursementId}&author=${reburse.author}&amount=${reburse.amount}&dateSubmitted=${reburse.dateSubmitted}&dateResolved=${reburse.dateResolved}&description=${reburse.description}&status=${reburse.status.status}&type=${reburse.type.type}&resolver=${reburse.resolver}`;
      console.log(queryString);
